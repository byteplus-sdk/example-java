...
import static byteplus.example.LocalTest.byteair.Constant.*;
...
//                .hosts(Collections.singletonList(LF_HOST))
                .schema("http")
//                .ak(AK)
//                .sk(SK)
                .useAirAuth()
    ...
    public static void main(String[] args) {
        concurrentWriteUserAndDone(buildDateList());
//        // 实时数据上传
//        writeDataExample();
//        // 并发实时数据上传
...
//        // wait until all tasks finished all timeout
//        concurrentHelper.waitAndShutdown();
//    }

    // 并发上传数据
    public static void concurrentWriteUserAndDone(List<LocalDate> dateList) {
        List<Map<String, Object>> dataList = MockHelper.mockDataList(2);
        String topic = Constant.TOPIC_USER;
        String stage = STAGE_INCREMENTAL_SYNC_DAILY;
        for (LocalDate date : dateList) {
            for (int i=0;i<1000;i++) {
                Option[] opts = writeOptions(stage, date);
                concurrentHelper.submitWriteRequest(dataList, topic, opts);
            }
        }
        ...
        // wait until all tasks finished all timeout
        concurrentHelper.waitAndShutdown();
        doneExample(dateList, topic, stage);
    }

    public static List<LocalDate> buildDateList() {
        List<LocalDate> dateList = new ArrayList<>();
        dateList.add(LocalDate.of(2021,12,20));
        dateList.add(LocalDate.of(2021,12,21));
        dateList.add(LocalDate.of(2021,12,22));
//        dateList.add(LocalDate.of(2021,12,4));
        return dateList;
    }

    // Write请求参数说明，请根据说明修改
    private static Option[] writeOptions(String stage, LocalDate date) {
        return new Option[]{
                // 必选. Write接口只能用于实时数据传输，此处只能填"incremental_sync_streaming"
                Option.withStage(stage),
    ...
    private static void doneExample(List<LocalDate> dateList, String topic, String stage) {
        Option[] opts = doneOptions(stage);
...
//    }
    ...
    // Done请求参数说明，请根据说明修改
    private static Option[] doneOptions(String stage) {
        return new Option[]{
